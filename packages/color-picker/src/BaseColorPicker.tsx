...
import { modifiers, normalizeKeyName } from '@lidojs/utils/keyboard';
...
export const BaseColorPicker = ({
    ...
    const handlePaste = (e: React.ClipboardEvent<HTMLInputElement>) => {
        ...
        const hexValue = `${text.replaceAll(/([^a-f0-9])/g, '')}`.slice(0, 6);
        updateValue(hexValue);
    ...
    const handleEyedropper = async () => {
        if (window.EyeDropper) {
            ...
            (inputRef.current as HTMLInputElement).value = new Color(result.sRGBHex).toHex();
            updateValue(new Color(result.sRGBHex).toHex());
        }
    };
    return (

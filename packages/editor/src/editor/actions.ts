...
import { Color, mergeWithoutArray } from '@lidojs/utils';
...
export const ActionMethods = (state: EditorState, query: CoreEditorQuery) => {
    ...
    return {
        ...
        ungroup(layerId: LayerId) {
            ...
            child.forEach((id) => {
                ...
                if (isTextLayer(layer) || isFrameLayer(layer)) {
                    ...
                    layer.data.props.image.position.x = layer.data.props.image.position.x * group.data.props.scale;
                    layer.data.props.image.position.y = layer.data.props.image.position.y * group.data.props.scale;
                }
            });
        ...
        addSvgLayer(url: string, boxSize: BoxSize, element: HTMLElement, parentId: LayerId = 'ROOT') {
            const layerId = getRandomId();
            const pageSize = state.pageSize;
            const ratio = pageSize.width / pageSize.height;
            const svgRatio = boxSize.width / boxSize.height;
            const w = ratio < svgRatio ? pageSize.width * 0.5 : pageSize.height * svgRatio * 0.5;
            const h = w / svgRatio;
            const colors: string[] = [];
            const list = element.getElementsByTagName('g');
            const defaultStrokeColor = element.getAttribute('stroke') || element.style.stroke || 'none';
            const defaultFillColor = element.getAttribute('fill') || element.style.fill || '#000000';
            if (list.length > 1) {
                for (let i = 0; i < list.length; i++) {
                    ...
                    if (id) {
                        ...
                        if (verifyId && verifyId.length >= 2) {
                            ...
                            if (paths.length > 0) {
                                const stroke = paths[0].getAttribute('stroke') || defaultStrokeColor;
                                const fill = paths[0].getAttribute('fill') || defaultFillColor;
                                if (stroke !== 'none') {
                                    colors[colorIndex] = new Color(stroke).toRgbString();
                                } else {
                                    colors[colorIndex] = new Color(fill).toRgbString();
                                }
                            }
                        }
                    }
                ...
                for (let j = 0; j < paths.length; j++) {
                    ...
                    if (stroke && stroke !== 'none' && !colors.includes(new Color(stroke).toRgbString())) {
                        colors.push(new Color(stroke).toRgbString());
                    } else if (fill && fill !== 'none' && !colors.includes(new Color(fill).toRgbString())) {
        ...
        addFrameLayer(boxSize: BoxSize, clipPath: string, parentId: LayerId = 'ROOT') {
            const layerId = getRandomId();
            const pageSize = state.pageSize;
            const ratio = pageSize.width / pageSize.height;
            const frameRatio = boxSize.width / boxSize.height;
            const scale =
                ratio > frameRatio ? (pageSize.height * 0.5) / boxSize.height : (pageSize.width * 0.5) / boxSize.width;
            const dl = deserializeLayer({
                type: {
        ...
        addVideoLayer({ url }: { url: string }, boxSize: BoxSize, parentId: LayerId = 'ROOT') {
            const layerId = getRandomId();
            const pageSize = state.pageSize;
            const ratio = pageSize.width / pageSize.height;
            const imgRatio = boxSize.width / boxSize.height;
                ...
                state.pages[imageEditor.pageIndex].layers[imageEditor.layerId].data.props = mergeWithoutArray(
                    state.pages[imageEditor.pageIndex].layers[imageEditor.layerId].data.props,
                    {
                        ...
                        video: imageEditor.video
                            ? {
                                  boxSize: {
                                      width: imageEditor.video.boxSize.width / (originalLayer.data.props.scale || 1),
                                      height: imageEditor.video.boxSize.height / (originalLayer.data.props.scale || 1),
                                  },
                                  position: {
                                      x: imageEditor.video.position.x / (originalLayer.data.props.scale || 1),
                                      y: imageEditor.video.position.y / (originalLayer.data.props.scale || 1),
                                  },

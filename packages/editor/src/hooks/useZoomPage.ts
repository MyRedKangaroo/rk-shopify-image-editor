...
export const useZoomPage = (
    ...
    const [pageTransform, setPageTransform] = useState<{ x: number; y: number; scale: number }>({
        scale: 1,
        x: 0,
        y: 0,
    });
    ...
    const pageZoomMove = useCallback(
        (change: number) => {
            if (frameRef.current && transformRef.current.isZoom && pageListRef.current) {
                const headerHeight = 70;
                const footerHeight = 72;
                const offset = 16;
                const containerWidth = window.innerWidth - offset * 2;
                const containerHeight = window.innerHeight - headerHeight - footerHeight - offset * 2;
                ...
                pageListRef.current[activePage].style.transform = getTransformStyle({
                    position: { x: x + changeX * (change - 1), y: y + changeY * (change - 1) },
                    scale: change,
                });
            }
        },
    ...
    const pageZoomEnd = useCallback(
        (change: number) => {
            if (frameRef.current && transformRef.current.isZoom && pageListRef.current) {
                ...
                const perfectX = (containerWidth - oldPageW) / 2;
                const perfectY = (containerHeight - oldPageH) / 2;
                const changeX = (pageW - oldPageW) / 2 - (perfectX === x ? 0 : x - perfectX) * (zoom - 1);
                const changeY = (pageH - oldPageH) / 2 - (perfectY === y ? 0 : y - perfectY) * (zoom - 1);

                const newX = Math.max(-(pageW - containerWidth), Math.min(x - changeX, 0));
                const newY = Math.max(-(pageH - containerHeight / 2), Math.min(y - changeY, containerHeight / 2));
    ...
    const handleZoomMove = useCallback(
        throttle((e: React.TouchEvent) => {
            ...
            const start = distanceBetweenPoints(transformRef.current.touchStart[0], transformRef.current.touchStart[1]);
    ...
    const handleMove = (e: React.TouchEvent) => {
        ...
        const containerWidth = window.innerWidth - offset * 2;
        const containerHeight = window.innerHeight - headerHeight - footerHeight - offset * 2;
        if (transformRef.current.isZoom || !transformRef.current.isMoving || containerWidth >= pageSize.width * scale)
            return;
        ...
        const x = Math.max(
            -(pageW - containerWidth),
            Math.min(pageTransform.x + clientX - transformRef.current.start.clientX, 0),
        );
        const y = Math.max(
            -(pageH - containerHeight / 2),
            Math.min(pageTransform.y + clientY - transformRef.current.start.clientY, containerHeight / 2),
        );
    ...
    const handleMoveEnd = () => {
        ...
        const pageW = pageSize.width * scale;
        const pageH = pageSize.height * scale;
        const x = Math.max(-(pageW - containerWidth), Math.min(pageTransform.x + moveX, 0));
        const y = Math.max(
            -(pageH - containerHeight / 2),
            Math.min(pageTransform.y + clientY - transformRef.current.start.clientY, containerHeight / 2),
        );
    ...
    const handleMovePageEnd = () => {
        ...
        const containerWidth = window.innerWidth - offset * 2;
        if (containerWidth >= pageSize.width * scale) {
            if (moveX <= -(window.innerWidth * 0.35) && activePage + 1 <= pageLength - 1) {
                pageContainerRef.current.style.transform = getTransformStyle({
        ...
        const updateSize = () => {
            if (frameRef.current) {
                ...
                if (isMobile) {
                    ...
                    const offsetTop = 16;
                    const y =
                        (window.innerHeight - pageSize.height * scale - headerHeight - footerHeight - offsetTop) / 2;
